...
void debuggerPrintValueInternalBlock(Function *, Type *, ELFBlock *, int, int, u32);
                         ...
                         u32 location)
{
  ...
  if(bitSize) {
    ...
    if(size == 1)
      ...
      debuggerPrintValueInternalBlock(f, m->type, m->location, m->bitSize,
...
void debuggerPrintValueInternalBlock(Function *f, Type *t, ELFBlock *loc,
                                int bitSize, int bitOffset, u32 objLocation)
{
  ...
  if(loc) {
    if(objLocation)
      ...
      location = elfDecodeLocationBase0(f, loc, &type);
  } else {
...
{
  debuggerPrintValueInternalBlock(f, o->type, o->location, 0, 0, 0);
...
void debuggerBreakList(int n, const char **args)
{
...
void debuggerRegisters(int n, const char **args)
{
...
void debuggerQuit(int n, const char **args)
{
  char buffer[10];
