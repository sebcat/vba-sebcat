...
int soundPaused = 1;
int soundPlay = 0;
int soundTicks = /*soundQuality*/ 2 * USE_TICKS_AS;
int SOUND_CLOCK_TICKS = /*soundQuality*/ 2 * USE_TICKS_AS;
...
void soundDirectSoundATimer()
{
  if(soundDSAEnabled) {
    if(soundDSFifoACount <= 16) {
      CPUCheckDMA(3, 2);
      if(soundDSFifoACount <= 16) {
        soundEvent16(FIFOA_L, 0);
        soundEvent16(FIFOA_H, 0);
        soundEvent16(FIFOA_L, 0);
        soundEvent16(FIFOA_H, 0);
        soundEvent16(FIFOA_L, 0);
        soundEvent16(FIFOA_H, 0);
...
void soundDirectSoundBTimer()
{
  if(soundDSBEnabled) {
    if(soundDSFifoBCount <= 16) {
      CPUCheckDMA(3, 4);
      if(soundDSFifoBCount <= 16) {
        soundEvent16(FIFOB_L, 0);
        soundEvent16(FIFOB_H, 0);
        soundEvent16(FIFOB_L, 0);
        soundEvent16(FIFOB_H, 0);
        soundEvent16(FIFOB_L, 0);
        soundEvent16(FIFOB_H, 0);
