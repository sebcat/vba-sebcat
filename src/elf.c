...
u32 elfDecodeLocationBase0(Function *f, ELFBlock *o, LocationType *type)
{
...
u8 *elfParseFunction(u8 *data, ELFAbbrev *abbrev, CompileUnit *unit,
                     Function **function);
void elfCleanUpFunction(Function *);
...
void elfParseTypeComplete(u8 *data, u32 offset, ELFAbbrev *abbrev,
    CompileUnit *unit, Type **type)
{
  switch(abbrev->tag) {
  case DW_TAG_typedef:
    {
...
Type *elfParseType(CompileUnit *unit, u32 offset)
{
  ...
  elfParseTypeComplete(data, offset, abbrev, unit, &type);
  return type;
}
...
bool elfReadProgram(ELFHeader *eh, u8 *data, size_t *size, bool parseDebug)
{
...
bool elfRead(const char *name, size_t *siz, FILE *f)
{
  ELFHeader *header;
  fseek(f, 0, SEEK_END);
  long size = ftell(f);
...
void elfCleanUpObject(Object *o)
{
  free(o->location);
}

void elfCleanUpFunction(Function *func)
{
...
void elfCleanUpAbbrev(ELFAbbrev **abbrevs)
{
  for(int i = 0; i < 121; i++) {
...
void elfCleanUpCompileUnit(CompileUnit *comp)
{
  elfCleanUpAbbrev(comp->abbrevs);
  free(comp->abbrevs);
